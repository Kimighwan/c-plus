...
typedef std::streamsize precsis;
format setFormat();
void restore(format f, precsis partial);
...
void BrassPlus::Withdraw(double amt) {
	...
	if (amt <= bal)
		Brass::Withdraw(amt);
	else if (amt <= bal + maxLoan - owesBandk)
	{
		double advance = amt - bal;
		owesBandk += advance * (1.0 + rate);
...
void restore(format f, precsis p) {
